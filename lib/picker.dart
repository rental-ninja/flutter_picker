...
typedef PickerItemBuilder = Widget? Function(BuildContext context, String? text, Widget? child, bool selected, int col, int index);
...
class Picker {
  ...
  Future<T?> showModal<T>(BuildContext context, {ThemeData? themeData, bool isScrollControlled = false, bool useRootNavigator = false, Color? backgroundColor, PickerWidgetBuilder? builder}) async {
  ...
  /// show dialog picker
  Future<List<int>?> showDialog(BuildContext context, {bool barrierDismissible = true, Color? backgroundColor, PickerWidgetBuilder? builder, Key? key}) {
    return Dialog.showDialog<List<int>>(
        ...
        builder: (BuildContext context) {
          ...
          final _cancel = PickerWidgetState._buildButton(context, cancelText, cancel, cancelTextStyle, true, theme, () {
          ...
          final _confirm = PickerWidgetState._buildButton(context, confirmText, confirm, confirmTextStyle, false, theme, () async {
...
class _PickerWidget<T> extends StatefulWidget {
  ...
  final bool isModal;
  _PickerWidget({Key? key, required this.picker, this.themeData, required this.isModal}) : super(key: key);
...
class PickerWidgetState<T> extends State<_PickerWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    if (!picker.hideHeader) {
      if (picker.builderHeader != null) {
        _body.add(picker.headerDecoration == null ? picker.builderHeader!(context) : DecoratedBox(child: picker.builderHeader!(context), decoration: picker.headerDecoration!));
      } else {
        _body.add(DecoratedBox(
          ...
          decoration: picker.headerDecoration ??
              BoxDecoration(
                ...
                color: picker.headerColor == null ? theme?.bottomAppBarTheme.color : picker.headerColor,
              ),
        ));
      }
    }
  ...
  List<Widget> _buildHeaderViews(BuildContext context) {
    ...
    final _cancel = _buildButton(context, picker.cancelText, picker.cancel, picker.cancelTextStyle, true, theme, () => picker.doCancel(context));
    if (_cancel != null) {
      items.add(_cancel);
    }
    ...
    final _confirm = _buildButton(context, picker.confirmText, picker.confirm, picker.confirmTextStyle, false, theme, () => picker.doConfirm(context));
    if (_confirm != null) {
      items.add(_confirm);
    }
  ...
  static Widget? _buildButton(BuildContext context, String? text, Widget? widget, TextStyle? textStyle, bool isCancel, ThemeData? theme, VoidCallback? onPressed) {
    if (widget == null) {
      String? _txt = text ?? (isCancel ? PickerLocalizations.of(context).cancelText : PickerLocalizations.of(context).confirmText);
      ...
      return TextButton(
          ...
          onPressed: onPressed,
          child: Text(_txt, overflow: TextOverflow.ellipsis, textScaleFactor: MediaQuery.of(context).textScaleFactor, style: textStyle));
    } else {
            ...
            child: _wait
                ...
                : StatefulBuilder(
                    builder: (context, state) {
                      ...
                      // 上一次是空列表
                      final _lastIsEmpty = scrollController[i].hasClients && !scrollController[i].position.hasContentDimensions;

                      final _length = adapter.length;
                      final _view = _buildCupertinoPicker(context, i, _length, adapter, _lastIsEmpty ? ValueKey(_length) : null);
                      ...
                      if (_lastIsEmpty || (!picker.changeToFirst && picker.selecteds[i] >= _length)) {
                        Timer(Duration(milliseconds: 100), () {
                          ...
                          if (scrollController[i].position.hasContentDimensions) {
                            ...
                            scrollController[i] = FixedExtentScrollController(initialItem: _index);
                            if (_keys[i] != null) {
                              _keys[i]!(() => null);
                            }
                          }
  ...
  Widget _buildCupertinoPicker(BuildContext context, int i, int _length, PickerAdapter adapter, Key? key) {
    return CupertinoPicker.builder(
      ...
      onSelectedItemChanged: (int _index) {
        ...
        if (picker.printDebug) print("onSelectedItemChanged. col: $i, row: $index");
...
abstract class PickerAdapter<T> {
  ...
  Widget makeText(Widget? child, String? text, bool isSel) {
    final theme = picker!.textStyle != null || picker!.state?.context == null ? null : Theme.of(picker!.state!.context);
    return Center(
        child: DefaultTextStyle(
            ...
            style: picker!.textStyle ??
                TextStyle(
                    ...
                    fontFamily: theme == null ? "" : theme.textTheme.titleLarge?.fontFamily,
                    fontSize: Picker.DefaultTextSize),
            child: child != null
                ? (isSel && picker!.selectedIconTheme != null
                    ...
                    : child)
                : Text(text ?? "", textScaleFactor: picker!.textScaleFactor, style: (isSel ? picker!.selectedTextStyle : null))));
  }
  ...
  Widget makeTextEx(Widget? child, String text, Widget? postfix, Widget? suffix, bool isSel) {
    ...
    final theme = picker!.textStyle != null || picker!.state?.context == null ? null : Theme.of(picker!.state!.context);
    Color? _txtColor = theme?.brightness == Brightness.dark ? Colors.white : Colors.black87;
    ...
    return Center(
        child: DefaultTextStyle(
            ...
            maxLines: 1,
            textAlign: picker!.textAlign,
            style: picker!.textStyle ?? TextStyle(color: _txtColor, fontSize: _txtSize, fontFamily: theme == null ? "" : theme.textTheme.titleLarge?.fontFamily),
            child: Wrap(
              children: items,
            )));
  }
...
class PickerDataAdapter<T> extends PickerAdapter<T> {
  ...
  Widget buildItem(BuildContext context, int index) {
    ...
    if (picker!.onBuilderItem != null) {
      final _v = picker!.onBuilderItem!(context, item.value.toString(), item.text, isSel, _col, index);
  ...
  _checkPickerDataLevel(List<PickerItem>? data, int level) {
    ...
    for (int i = 0; i < data.length; i++) {
      if (data[i].children != null && data[i].children!.length > 0) _checkPickerDataLevel(data[i].children, level + 1);
    }
...
class DateTimePickerAdapter extends PickerAdapter<DateTime> {
  ...
  DateTimePickerAdapter({
    ...
    _needUpdatePrev = day < month || day < _columnType.indexWhere((element) => element == 0);
    if (!_needUpdatePrev) {
  ...
  // 判断是否存在秒
  bool existSec() {
    final _columns = customColumnType == null ? columnType[type] : customColumnType!;
  ...
  int getLength() {
    ...
    switch (_type) {
      case 3: // hour
        if ((minHour != null && minHour! >= 0) || (maxHour != null && maxHour! <= 23)) return (maxHour ?? 23) - (minHour ?? 0) + 1;
        break;
      case 4: // minute
        if (minuteInterval != null && minuteInterval! > 1) return v ~/ minuteInterval!;
        break;
      case 7: // hour am/pm
        if ((minHour != null && minHour! >= 0) || (maxHour != null && maxHour! <= 23)) if (_colAP < 0) {
  ...
  Widget buildItem(BuildContext context, int index) {
    ...
    switch (colType) {
      ...
      case 4:
        ...
        break;
      case 6:
        final apStr = strAMPM ?? PickerLocalizations.of(context).ampm ?? const ['AM', 'PM'];
        _text = "${apStr[index]}";
        break;
      case 7:
  ...
  void doShow() {
    ...
    for (int i = 0; i < _maxLevel; i++) {
      ...
      switch (colType) {
        ...
        case 6:
          // am/pm
          picker!.selecteds[i] = (sh > 12 || (sh == 12 && (value!.minute > 0 || value!.second > 0))) ? 1 : 0;
          break;
        case 7:
  ...
  bool _verificationMinMaxValue() {
    ...
    if (_minV != null && (value!.millisecondsSinceEpoch < _minV.millisecondsSinceEpoch)) {
      value = _minV;
      return true;
    } else if (_maxV != null && value!.millisecondsSinceEpoch > _maxV.millisecondsSinceEpoch) {
      value = _maxV;
      return true;
    }
    return false;
